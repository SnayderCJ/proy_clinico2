...
def calendario_citas_mejorado(request):
    ...
    citas_mes = (
        CitaMedica.objects.filter(fecha__year=year, fecha__month=month)
    ...
    horarios = HorarioAtencion.objects.filter(activo=True).order_by(
...
def horarios_disponibles_mejorado(request):
    ...
    if request.method == "GET":
        ...
        try:
            ...
            return JsonResponse(
                {"error": "Formato de fecha inválido (YYYY-MM-DD)"}, status=400
            )
        ...
        citas_ocupadas = (
            CitaMedica.objects.filter(fecha=fecha)
            .exclude(estado=EstadoCitaChoices.CANCELADA)
        ...
        for horario in horarios:
            ...
            while hora_actual < hora_fin:
                ...
                if (
                    horario.intervalo_desde
                    and horario.intervalo_hasta
                    and horario.intervalo_desde
                    <= hora_actual.time()
                    < horario.intervalo_hasta
                ):
                ...
                if fecha == date.today():
                    hora_limite = timezone.now() + timedelta(
                        hours=1
        ...
        try:
            ...
            with transaction.atomic():
                ...
                try:
                    ...
                    return JsonResponse(
                        {"success": False, "error": "Formato de fecha u hora inválido"},
                        status=400,
                    )
                ...
                if errores:
                    return JsonResponse(
                        {"success": False, "error": "; ".join(errores)}, status=400
                    )

                # Verificar disponibilidad final
                cita_existente = (
                    CitaMedica.objects.filter(fecha=fecha, hora_cita=hora)
                    .exclude(estado=EstadoCitaChoices.CANCELADA)
                    .exists()
                )
                ...
                cita_mismo_dia = (
                    CitaMedica.objects.filter(paciente=paciente, fecha=fecha)
                    .exclude(estado=EstadoCitaChoices.CANCELADA)
                    .exists()
                )
...
def validar_cita(fecha, hora, paciente):
    ...
    dia_semana = dias_semana_map.get(dia_semana_num)
    horarios_dia = HorarioAtencion.objects.filter(dia_semana=dia_semana, activo=True)
    ...
    for horario in horarios_dia:
        if horario.hora_inicio <= hora <= horario.hora_fin:
            ...
            if (
                horario.intervalo_desde
                and horario.intervalo_hasta
                and horario.intervalo_desde <= hora < horario.intervalo_hasta
            ):
    ...
    if hora.minute not in [0, 30]:
        errores.append(
            "Las citas solo se pueden agendar cada 30 minutos (ej: 09:00, 09:30)"
        )
                    ...
                    try:
                        ...
                        if nueva_fecha != cita.fecha or nueva_hora != cita.hora_cita:
                            ...
                            errores = validar_cita(
                                nueva_fecha, nueva_hora, cita.paciente
                            )
                            if errores:
                                return JsonResponse(
                                    {"success": False, "error": "; ".join(errores)},
                                    status=400,
                                )
                            ...
                            cita_existente = (
                                CitaMedica.objects.filter(
                                    fecha=nueva_fecha, hora_cita=nueva_hora
                                )
                                .exclude(id=cita.id)
                                .exclude(estado=EstadoCitaChoices.CANCELADA)
                                .exists()
                ...
                return JsonResponse(
                    {
                        "success": True,
                        "mensaje": (
                            f'Cita actualizada exitosamente. Cambios: {", ".join(cambios)}'
                            if cambios
    ...
    return JsonResponse(
        {
            "cita": {
                ...
                "puede_cancelar": cita.estado
                not in [EstadoCitaChoices.COMPLETADA, EstadoCitaChoices.CANCELADA],
            }
        }
    )
    ...
    if request.method == "DELETE":
        try:
            ...
            if cita.estado == EstadoCitaChoices.CANCELADA:
                return JsonResponse(
                    {"success": False, "error": "La cita ya está cancelada"}, status=400
                )
            ...
            if cita.fecha == date.today():
                ...
                cita.observaciones = (
                    f"CANCELADA EL MISMO DÍA: {motivo}. {cita.observaciones or ''}"
                )
...
def buscar_pacientes_mejorado(request):
    ...
    if len(query) < 2:
        return JsonResponse(
            {"pacientes": [], "mensaje": "Ingrese al menos 2 caracteres para buscar"}
        )
    ...
    for paciente in pacientes:
        # Obtener última cita
        ultima_cita = (
            CitaMedica.objects.filter(paciente=paciente)
            .order_by("-fecha", "-hora_cita")
            .first()
        )
        ...
        pacientes_data.append(
            {
                ...
                "edad": paciente.edad,
                "ultima_cita": (
                    ultima_cita.fecha.strftime("%Y-%m-%d")
                    if ultima_cita
                    else "Sin citas"
                ),
...
def estadisticas_calendario(request):
    ...
    citas_mes = CitaMedica.objects.filter(fecha__year=year, fecha__month=month)
    ...
    horarios = (
        citas_mes.values("hora_cita")
        .annotate(total=models.Count("hora_cita"))
        .order_by("-total")[:5]
    )
